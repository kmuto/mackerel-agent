...
func generateValues_clockup(generators []metrics.Generator) []*metrics.ValuesCustomIdentifier {
	...
	go func() {
		...
		for _, g := range generators {
			...
			go func(g metrics.Generator) {
				...
				startedAt := timejump.Now()
				values, err := g.Generate()
				if seconds := ((timejump.Now().Sub(startedAt)) / time.Second); seconds > 120 {
